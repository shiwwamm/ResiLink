# benchmark.py — SAFE: No execution on import
import os, time, csv
...
# ← NO CODE BELOW THIS LINE (except if __name__ == "__main__")
...
if __name__ == "__main__":
    ...
    for topo in TOPOLOGIES:
        ...
        for name, func in [
            ...
            results.append({
                "Topology": topo, "Method": name, "Final_TPut": f"{final_cut*10:.1f}",
                "Gain_%": f"{gain:.1f}", "Links": len(added), "Time_s": f"{time.time()-start:.1f}"
            })
        ...
        results.append({
            "Topology": topo, "Method": "RL-GNN", "Final_TPut": f"{final_cut*10:.1f}",
            "Gain_%": f"{gain:.1f}", "Links": len(added), "Time_s": f"{time.time()-start:.1f}"
        })
