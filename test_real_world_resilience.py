...
class RealWorldResilienceTest:
    ...
    def test_single_topology(self, topology_id):
        ...
        try:
            ...
            print("Verifying SDN controller API...")
            for i in range(12):  # Try for 60 seconds
            	try:
            	    import requests
            	    resp = requests.get('http://localhost:8080/stats/switches', timeout=5)
            	    if resp.status_code == 200:
            	    	print(f"API ready! Found {len(resp.json())} switches")
            	    break
            	except:
            	    print(f"   API not ready yet... ({i+1}/12)")
            	    time.sleep(5)
            else:
            	print("Warning: API still unreachable, but proceeding anyway...")
